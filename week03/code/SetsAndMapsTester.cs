...
public static class SetsAndMapsTester
{
    ...
    private static void DisplayPairs(string[] words)
    {
        ...
        HashSet<string> reversePairSet = new HashSet<string>();

        foreach (string word in words)
        {
            if (pairSet.Contains($"{word[1]}{word[0]}") && word[0] != word[1] && reversePairSet.Contains($"{word} & {word[1]}{word[0]}") == false)
            {
                reversePairSet.Add($"{word[1]}{word[0]} & {word}");
            }
        }
        foreach (var item in reversePairSet)
        {
